...
RESTORED 2026-09-07. Deleted the same evening in linafish#79 as "orphan — never imported",
which was true of the imports and false of the design. memory/notes_2026-08-31 (the less
complicated build, §1): these five attractors and glyph_evolution are the two halves of RCP
as a NEGOTIATION SURFACE — "the base makes it intelligible; the rest is not ours to build" —
and the seeds are the permanent surface that must never drift so the divergence above it
is safe. We shipped both halves and never wrote the contract between them. A module with no
importer is not dead when the design says it is the base; it is UNWIRED, which is the
opposite failure. Captain, same day: "not building everything from scratch … add our novel
ideas and context." Deleting the base to tidy a lint report is building from scratch in
reverse.
