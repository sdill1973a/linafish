...
def _build_money_line(formations, crystal_map: dict) -> str:
    ...
    top_formations = sorted(formations, key=lambda x: getattr(x, "compression_score", 0.0), reverse=True)[:7]
...
def _build_portrait(formations, total_crystals: int, total_docs: int, crystal_map: dict = None) -> str:
    ...
    for f in sorted(formations, key=lambda x: getattr(x, "compression_score", 0.0), reverse=True)[:shown]:
    ...
    candidates = sorted(formations, key=lambda x: getattr(x, "compression_score", 0.0), reverse=True)[:7]
    best_sample = None
...
def _cognitive_portrait(formations, total_docs: int, crystal_map: dict = None) -> Optional[str]:
    ...
    if len(formations) >= 3:
        sorted_f = sorted(formations, key=lambda x: getattr(x, "compression_score", 0.0), reverse=True)
        themes = []
        for f in sorted_f[:4]:
...
def _build_portrait_prompt(formations, total_docs: int, crystal_map: dict = None) -> Optional[str]:
    ...
    if formations:
        ...
        formation_section += chr(10).join(
            ...
            for f in sorted(formations, key=lambda x: -getattr(x, "compression_score", 0.0))[:5]
        )
    else:
        formation_section = (
...
def explain_the_why(total_docs: int, total_crystals: int, formations: list,
    ...
    n_formations = len(formations)
    top_f = sorted(formations, key=lambda x: getattr(x, "compression_score", 0.0), reverse=True)[0]
...
def go(
    ...
    if engine.formations:
        ...
        sorted_f = sorted(engine.formations, key=lambda x: getattr(x, "compression_score", 0.0), reverse=True)
